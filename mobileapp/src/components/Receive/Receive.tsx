...
export default function Receive({ address }: ReceiveProps) {
  ...
  const onShare = async () => {
    try {
      await Share.share({
        message:
          'Hi! I want to share my Sapphire wallet address with you: ' +
          address +
          '.',
      })
    } catch (error: any) {}
  ...
  return (
    <Layout style={styles.container}>
      ...
      <Text style={{ marginBottom: 2 * vh }}>
        Show this QR code to receive ETH or NFTs
      </Text>
      <QRCode value={address} size={75 * vw} />
      <Text style={{ marginTop: 2 * vh }}>
        Alternatively, share your wallet address:
      </Text>
      <Button
        testID="share-button"
