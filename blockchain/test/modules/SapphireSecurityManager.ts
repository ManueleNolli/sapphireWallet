...
describe('SapphireSecurityManager', function () {
  ...
  describe('Integration, relayed tx', async function () {
    beforeEach(async function () {
      ...
      await infrastructure.dappRegistry.connect(deployer).setAuthorised(await infrastructure.argentModule.getAddress(), true)
    })
    ...
    it('executeRecovery', async function () {
      ...
      // before owner
      expect(await IWallet__factory.connect(walletAccountAbstraction, deployer).owner()).to.be.equal(walletOwner.address)
