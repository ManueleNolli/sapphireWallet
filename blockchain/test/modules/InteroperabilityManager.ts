...
import { ArgentModule, BaseWallet__factory, SapphireNFTs__factory } from '../../typechain-types'
...
async function utilsSignTransaction(
  ...
  const methodData = argentModule.interface.encodeFunctionData('bridgeCall', [signerContractWallet, transaction])

  const chainIdTX = (await ethers.provider.getNetwork()).chainId
  ...
  return await argentModule
    .connect(deployer)
    .execute(signerContractWallet, methodData, nonce, signedTransaction, 0, gasLimit, ZeroAddress, ZeroAddress)
}
...
describe('InteroperabilityManager', function () {
  ...
  describe('bridgeCall', async function () {
    it('Should revert if value is zero and transaction data has zero bytes', async function () {
      ...
      const expectedError = ethers.solidityPacked(['string'], ['InteroperabilityManager: Invalid transaction. Only value or data can be set'])
    ...
    describe('BRIDGE_ETH callType', async function () {
      it('Should revert if value is valid but wallet has no enough balance', async function () {
        ...
        const returnData = eventArgs[2]
        const expectedError = ethers.solidityPacked(['string'], ['InteroperabilityManager: Not enough balance'])
        expect(returnData).to.include(expectedError.slice(2))
      })

      it('Should be valid if value is something', async function () {
        const argentModuleBalanceBefore = await ethers.provider.getBalance(await infrastructure.argentModule.getAddress())
        ...
        expect(eventArgs2[0]).to.equal(0)
        expect(eventArgs2[1]).to.equal(walletAccount1Address)
        expect(eventArgs2[2]).to.equal(BridgeCallType.BRIDGE_ETH)
        expect(eventArgs2[3]).to.equal(account2.address)
        expect(eventArgs2[4]).to.equal(parseEther('10'))
        expect(eventArgs2[5]).to.equal('0x')

        // EVENT 3= TransactionExecuted
        const event3 = txResponse?.logs[3] as EventLog
        const eventName3 = event3?.fragment.name
        expect(eventName3).to.equal('TransactionExecuted')
        const eventArgs3 = event3?.args
        expect(eventArgs3[0]).to.equal(walletAccount1Address)
        expect(eventArgs3[1]).to.be.true

        const argentModuleBalanceAfter = await ethers.provider.getBalance(await infrastructure.argentModule.getAddress())
        expect(argentModuleBalanceAfter).to.equal(argentModuleBalanceBefore + parseEther('10'))
      })
    })
    ...
    describe('BRIDGE_NFT callType', async function () {
      ...
      it('Should revert if safeTransferFrom param to is not the InteroperabilityManager', async function () {
        ...
        const data = SapphireNFTs__factory.createInterface().encodeFunctionData('safeTransferFrom(address,address,uint256)', [
          account1.address,
          account2.address,
          0,
        ])
      ...
      it('Should revert if NFT transfer fails', async function () {
        ...
        const sapphireNFTDeployment = await SapphireNFT.deploy(await deployer.getAddress())
        ...
        const expectedError = ethers.solidityPacked(['string'], ['InteroperabilityManager: NFT transfer to InteroperabilityManager failed'])
        expect(returnData).to.include(expectedError.slice(2))
      })
      ...
      it('Should be valid', async function () {
        ...
        // EVENT 2 = chain Added
        // EVENT 3 = BridgeCall
        const event1 = txResponse?.logs[3] as EventLog
        ...
        // EVENT 4 = TransactionExecuted
        const event2 = txResponse?.logs[4] as EventLog
        const eventName2 = event2?.fragment.name
        ...
        expect(ownerAfter).to.equal(await infrastructure.argentModule.getAddress())
      })
    })
    ...
    describe('DEST callType', async function () {
      it('Should revert if value is not zero', async function () {
        ...
        const returnData = eventArgs[2]
        const expectedError = ethers.solidityPacked(['string'], ['InteroperabilityManager: Invalid transaction. Value can not be set in type DEST'])
        expect(returnData).to.include(expectedError.slice(2))
      })
      ...
      it('Should revert if data is something but no signature', async function () {
        ...
        const returnData = eventArgs[2]
        const expectedError = ethers.solidityPacked(['string'], ['InteroperabilityManager: Invalid transaction. Signature must be set in type DEST'])
        expect(returnData).to.include(expectedError.slice(2))
      })
    ...
    it('Should add the chainId to the wallet', async function () {
      const argentModuleBalanceBefore = await ethers.provider.getBalance(await infrastructure.argentModule.getAddress())
      ...
      const wallet = BaseWallet__factory.connect(walletAccount1Address, deployer)
      const chainsArray = await wallet.getChains()

      expect(chainsArray.length).to.equal(2)

      expect(chainsArray[0]).to.equal((await ethers.provider.getNetwork()).chainId)

      expect(chainsArray[1]).to.equal(chainID)
    })
  })
