...
export default async function deployBaseChain() {
  ...
  const result = await deployInfrastructure(deployer).then(updateHardhatENV).then(printInfrastructure)
  ...
  await updateExternalEnv('../backend/wallet-factory/.env', walletFactoryBackendEnv)
